...
export default function CustomizedAccordions(props) {
    ...
    const [expandedIndex, setExpandedIndex] = useState(props.content[0] ? props.content[0]._id : '');
...
const documentTutorialDialoghandle = () => {
  ...
  return (
      <div>
          <VideoTutorialDialog handleShow={videoTutorialDialog} handleDialog={handleDialog} />
          <DocumentTutorialDialog handleShow={documentTutorialDialog} handleDialog={handleDialog} />
          <AssignmentDialog handleShow={assignmentDialog} handleDialog={handleDialog} /> 
      {content.map((con, i) => (
        <Accordion
          square
        ...
        >
          <AccordionSummary aria-controls="panel1d-content">
            <Typography> {con.name} #{i+1}, <small> {con.content.length} contents </small> </Typography>
          </AccordionSummary>
                  ...
                  <div>      
                                ...
                                { con.content.map((i, j) => (
                                  <ListItem onClick={() => console.log(i.contentId)} button>
                                    <ListItemIcon>
                                      {
                                         i.reference == "Video" ? <Movie /> :  i.reference == "Document" ? <Description /> : <Assignment/>
                                      }
                                    </ListItemIcon>
                                    <ListItemText
                                      primary={`${i.contentName}`}
                                    />
                                  </ListItem>
                                ))}
                              </List>
              <Button variant="contained" size="small" color="primary" onClick={videoTutorialDialoghandle}>
                Add Video
              </Button>
              <Button variant="contained" size="small" color="primary" style={{margin: "0 5px"}} onClick={documentTutorialDialoghandle}>
                Add Document
              </Button>
              <Button variant="contained" size="small" color="primary" onClick={AssignmentDialogHandle}>
                Add Assignment
              </Button>
            </div>
          </AccordionDetails>
